...
func (r *Room) assembleAwardsLocked() []AwardData {
	...
	if r.slots[0].pub != nil {
		host = r.sessionStats[r.slots[0].pub.UserID]
	} else {
	...
	if marathon != nil {
		add("marathon", marathon, durShort(marathon.TotalWatch))
	}
	if ghost != nil {
		add("ghost", ghost, "")
	}
	if faithful != nil {
		add("faithful", faithful, durShort(faithful.TotalWatch))
	}
	if loudest != nil {
		add("loudest", loudest, fmt.Sprintf("%d", loudest.StingerPlays))
	}
